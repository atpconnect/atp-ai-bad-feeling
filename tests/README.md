...
```bash
tests/rehearse.sh                    # every scenario, against the model stub, ~2 minutes
...
| `happy` | All five recordings worked. Also covers `notes.html`, the phone-sized talking-points companion built alongside the deck. |
...
| `fullsize` | Transcripts inflated to 8,500 words a station, above the ~6,300 the real ones came to, and timed. |
...
| `hints` | Pilot-flagged moments are found in real transcripts and never in a fallback, decoy phrasings are ignored, a whole session on one speaker line flags nothing, and the coverage check reports what landed without rejecting anything. |
...
**The prompt was passed as a shell argument, which breaks at exactly the wrong moment.** `claude -p "$(cat prompt)"` works fine on a 3KB test transcript. Linux caps a single argv string at 128KB (`MAX_ARG_STRLEN`, 32 pages, independent of the much larger `ARG_MAX`), the rehearsal's full-length fixtures come to about 285KB, and the five real transcripts came to 166KB. So the tool worked in every small test and would have died with `Argument list too long` on the night, with a room full of people waiting. Found by `fullsize`. Fixed by piping the prompt on stdin.
...
**Synthesis takes between 69 and 379 seconds, and the coordinator checklist said 60.** Four rehearsal runs came in at 69, 102, 105 and 164 seconds. On the night the primary never returned inside its 180 second deadline, and a later run of it against the same transcripts took 379. The important part is not the median, it is that **the slowest run was on the smallest input**: latency varies more than twofold for reasons that have nothing to do with how much we ask it to read, so it cannot be shortened by trimming transcripts. The run of show now gets a three-minute slice rather than a one-minute one, which moves the end-to-end worst case from roughly 8 minutes to roughly 10. Had we measured once and written that number down, we would have planned against 105 seconds and been wrong by a minute on the night.

**A 600-word test transcript flatters everything.** The fixtures are written to be readable. Real stations produced about 6,300 words each on the night, and the rehearsal pads to 8,500. `tests/inflate.py` pads the fixtures to that length so the timing rep measures the real thing; without it, two of the four findings above stay hidden.

**Pilot-flagged moments were found by splitting on speaker turns, and the real transcripts have exactly one.** The matcher looked for an intent word and a target word inside the same speaker turn, which is a fine definition of "the same breath" right up until you meet the recordings. Every station was captured on one laptop in a loud room, so Teams attributed all 46 minutes to a single speaker and the entire transcript arrived as one turn. The two words then co-occurred by chance in all five, and the tool reported two flagged moments on a night when a sentence-level scan finds that **nobody flagged anything at all**. One of the two false positives was the Swamp Planet pilot explaining this very tool to the room, *"going to a PowerPoint presentation"*, sitting near *"the library here has to get up"*. Fixed by requiring the two halves to be grammatically joined rather than merely nearby, and by refusing to name a speaker when the file cannot support one. The deeper lesson is that the fixtures were **better formed than reality**: they had tidy per-speaker turns because they were written by hand, and that structure was the one thing the feature depended on. `tests/fixtures/transcripts/one-speaker-teams.md` is now a real trimmed transcript, kept precisely so that shape is always in the suite.
