...
**One phrase, not two nearby words.** The intent and the destination have to be grammatically joined: an intent, a few words, a preposition, then the target. This is the part the real night rewrote. The first version split the transcript into speaker turns and asked whether a turn contained an intent word *and* a target word. It passed every fixture and then failed completely on the actual recordings, because Teams attributed each 46-minute station to a **single speaker** (one laptop, loud room, exactly as the expected transcripts predicted), so every transcript was one enormous turn and the two words co-occurred by chance in all five. It reported two flags on a night when nobody flagged anything, including on the Swamp Planet pilot describing this very tool to the room, *"going to a PowerPoint presentation"*, because *"the library here has to get up"* happened to sit nearby. Matching is now format-independent, and `tests/fixtures/transcripts/one-speaker-teams.md` is a real trimmed transcript kept as a fixture so that shape is always exercised.

**The substance is never in the flag.** A pilot says "I'd like to see *that* in the deck", so the pronoun points backwards. Each hint carries the preceding ~1100 characters, and the deck screen it produces is judged against them.

**It never invents a speaker.** Attribution is only reported when the transcript has enough speaker lines to mean anything; otherwise the hint says "someone in this station" rather than naming the meeting host that Teams pinned the whole room on.
