...
# The method, reliability, and receipts screens are Fleet Command's own words, not the
# model's, so their points live here as named constants: one copy read by points_html()
# for the deck itself and by notes_html() for the presenter's talking-points card, rather
# than the same six bullets typed out twice and drifting apart the first time one is edited.
METHOD_POINTS = [
    ...
    return [
        ...
        },
        {
            ...
            "body":
                ...
                + points_html(RELIABILITY_POINTS),
        },
    ]
...
def _trim(text, limit=140):
    """The first sentence of `text`, hard-trimmed at a word boundary only if that
    sentence alone still overruns `limit`. A talking-points card wants the clause
    that carries the number or the example, not the whole 560-character paragraph."""
    text = text.strip()
...
def notes_html(data, screens, mode):
    """A phone-sized, bullet-only companion to the deck.

    Fleet Command did not sit in any of the five stations and will not have heard
    any of this before walking on stage with it, so the talking points have to come
    from the same data the deck does, not be written ahead of time. Built from the
    default (unvoiced) text only: the numbers are identical across voices, so there
    is nothing a character voice adds that belongs on the presenter's own phone.
    """
    by_id = {s["id"]: s for s in screens}
    STATUS = {
        ("transcript", "demo"): "MOCK TRANSCRIPT", ("transcript", "seeded"): "TRANSCRIPT",
        ("transcript", "live"): "TRANSCRIPT",
        ("fallback", "seeded"): "PREVIEW — not yet run",
        ("fallback", "demo"): "FALLBACK — not a transcript",
        ("fallback", "live"): "FALLBACK — not a transcript",
    }

    fixed_points = {"method": METHOD_POINTS, "reliability": RELIABILITY_POINTS,
                     "receipts": RECEIPTS_POINTS}

    def fixed_card(sid):
        s = by_id[sid]
        items = "".join(f'<li><b>{e(p["t"])}</b><br>{e(_trim(p["d"]))}</li>'
                         for p in fixed_points[sid])
        return (f'<section class="card fixed"><h2>{e(s["kicker"])}</h2>'
                f'<p class="title">{e(s["title"])}</p>'
                f'<p class="note">Your own script, not the model’s — just prompts.</p>'
                f'<ul>{items}</ul>'
                f'<p class="takeaway">Leave them with: {e(s["takeaway"])}</p></section>')

    def station_card(st):
        status = STATUS[(st["source"], mode)]
        cls = "flagged" if st["source"] == "fallback" else ""
        items = "".join(f'<li><b>{e(p["t"])}</b><br>{e(_trim(p["d"]))}</li>' for p in st["points"])
        return (f'<section class="card {cls}"><h2>{e(st["name"])} <span class="tag">{status}</span></h2>'
                f'<p class="title">{e(st["theme"])}</p>'
                f'<p class="lede">{e(st["lede"])}</p>'
                f'<ul>{items}</ul>'
                f'<p class="takeaway">Leave them with: {e(st["takeaway"])}</p></section>')

    def patterns_card():
        items = "".join(f'<li><b>{e(p["t"])}</b><br>{e(_trim(p["d"]))}</li>' for p in data["patterns"])
        return (f'<section class="card"><h2>Across all five</h2>'
                f'<p class="title">What nobody in one room could see</p>'
                f'<ul>{items}</ul>'
                f'<p class="takeaway">Close with: {e(data["closing_line"])}</p></section>')

    cards = [fixed_card("method"), fixed_card("reliability")]
    cards += [station_card(st) for st in data["stations"]]
    cards += [patterns_card(), fixed_card("receipts")]
...
<title>Throne Room — talking points</title>
<style>
  :root { color-scheme: light dark; padding-top: env(safe-area-inset-top, 0px);
          padding-bottom: env(safe-area-inset-bottom, 0px); }
  body { margin: 0; background: #f6f5f2; color: #1a1a1a; font: 16px/1.45
         -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif; padding: 14px 16px 40px; }
  h1 { font-size: 1.05rem; margin: 0 0 4px; }
  .sub { color: #666; font-size: 0.85rem; margin: 0 0 18px; }
  .card { background: #fff; border: 1px solid #e2ded4; border-radius: 10px;
          padding: 14px 16px; margin-bottom: 14px; }
  .card.flagged { border-left: 4px solid #b5720a; }
  .card h2 { font-size: 0.78rem; text-transform: uppercase; letter-spacing: .04em;
             color: #7a6a4a; margin: 0 0 4px; display: flex; flex-wrap: wrap;
             justify-content: space-between; align-items: baseline; gap: 4px 8px; }
  .tag { font-size: 0.68rem; color: #b5720a; text-transform: none; letter-spacing: 0; }
  .title { font-weight: 600; margin: 0 0 8px; }
  .lede { margin: 0 0 10px; color: #444; }
  .note { margin: 0; color: #888; font-style: italic; }
  ul { margin: 0 0 10px; padding-left: 1.1em; }
  li { margin-bottom: 8px; }
  .takeaway { margin: 0; padding-top: 8px; border-top: 1px dashed #ddd; font-weight: 600; }
  @media (prefers-color-scheme: dark) {
    body { background: #17171a; color: #eee; }
    .card { background: #222226; border-color: #38383e; }
    .lede { color: #bbb; }
    .takeaway { border-top-color: #3a3a3f; }
  }
</style>
</head>
<body>
<h1>Throne Room — talking points</h1>
<p class="sub">Bullets only, in presentation order. Not what's on screen — what to say about it.</p>
""" + "\n".join(cards) + """
</body>
</html>
"""
...
def main():
    ...
    for spec in args.voice:
        ...
        order.append(name)
        voice_data[name] = vdata

    screens = decks["default"]

    # The talking-points notes are for whoever is presenting, not the audience, so they
    # follow the presenter's own phrasing rather than a character voice: if a "direct"
    # voice was requested and validated this run, its wording carries the notes; a voice
    # that failed or was never asked for falls back to the same default text the deck
    # uses, exactly like any other voice failure in this file.
    notes_source = voice_data.get("direct", data)
    ...
    notes_out = out.parent / "notes.html"
    notes_tmp = notes_out.with_suffix(notes_out.suffix + ".tmp")
    notes_tmp.write_text(notes_html(notes_source, screens, args.mode))
    notes_tmp.replace(notes_out)
    ...
    print("  notes.html: " + ("presenter's own phrasing (direct)" if "direct" in voice_data
                               else "default phrasing (pass --voices=direct for the presenter's own)"))
    return 0
